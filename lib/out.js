...
function url(input,output) {
    ...
    out.push('',['url parsed',ansi.underline.open + i + ansi.underline.close],[
...
function ip(input,output) {
    ...
    for ( var ip in event) {
        i++;
        out.push([ip,cyan + event[ip].counter + close]);
    }
    if (i == 0) {
        return [];
    }
    out
            .push('',['unique ip',
                    ansi.underline.open + i + ansi.underline.close],'');
    return out;
}
...
function avg(input,output) {
    ...
    return [output,['total',cyan + event.what + close],
            ['average',cyan + (event.what / event.total) + close],
            ['max',cyan + event.max + close],['min',cyan + event.min + close],
            ['']];
}
